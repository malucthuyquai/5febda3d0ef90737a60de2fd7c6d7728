...
+ (id)databaseWithPath:(NSString *)inPath andKey:(NSString *)key{
    return FMDBReturnAutoreleased([[self alloc] initWithPath:inPath andKey:key]);
}
...
- (id)initWithPath:(NSString *)inPath andKey:(NSString *)key{
    assert(sqlite3_threadsafe()); // whoa there big boy- gotta make sure sqlite it happy with what we're going to do.
    
    self = [super init];
    
    if (self) {
        _databasePath       = [inPath copy];
...
- (BOOL)openWithKey:(NSString *)key_ {
    if (_db) {
        return YES;
    }
    ...
    const char* key = [key_ UTF8String];
...
- (BOOL)openWithFlags:(int)flags andKey:(NSString *)key_ {
    int err = sqlite3_open_v2((_databasePath ? [_databasePath fileSystemRepresentation] : ":memory:"), &_db, flags, NULL /* Name of VFS module to use */);
    if(err != SQLITE_OK) {
        NSLog(@"error opening!: %d", err);
        return NO;
