...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {    
    ...
    if(!adView) {
        ...
        adView.view.frame = window.bounds;
        [viewController.view addSubview:adView.view];
    }
...
-(void)iRateDidOpenAppStore {
    [Flurry logEvent:@"Open Appstore For iRate"];
}

-(void)showSplash {
    if (!splash) {
        splash = [[UIImageView alloc] initWithFrame:window.bounds];
        CGSize s = [UIScreen mainScreen].bounds.size;
        if (s.height/s.width <= 1.5) {
