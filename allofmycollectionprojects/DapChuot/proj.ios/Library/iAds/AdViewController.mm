...
- (void)adViewDidReceiveAd:(GADBannerView *)view
{
    ...
    if (view.tag == 1) {
        adView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, [UIScreen mainScreen].bounds.size.height - view.frame.size.height/2);
    } else {
    ...
    g->postNotification(kNotificationAdmobShow);
}
...
- (void)adView:(GADBannerView *)view didFailToReceiveAdWithError:(GADRequestError *)error
{
    ...
    g->postNotification(kNotificationAdmobHide);
}
