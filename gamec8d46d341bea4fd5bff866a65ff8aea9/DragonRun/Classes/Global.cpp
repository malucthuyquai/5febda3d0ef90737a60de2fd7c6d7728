...
void Global::logEvent(std::string eventName) {
    global->flurryEventName = eventName;
    notify->postNotification(kNotifyFlurryLogEvent);
}
...
std::string Global::getKeyBestScore() {
    if (global->level == LEVEL_NORMAL) return kBestScore1_;
    else if (global->level == LEVEL_HARD) return kBestScore2_;
    else if (global->level == LEVEL_SUPPERHARD) return kBestScore3_;
    else return kBestScore4_;
}
