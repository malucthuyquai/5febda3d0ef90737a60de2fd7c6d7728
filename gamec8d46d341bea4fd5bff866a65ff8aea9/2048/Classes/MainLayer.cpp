...
//    hightSprite->setTag(M_TAG_HIGH_SCORE);
    ...
    TTFConfig configReset(M_FONT_CLEARSANS,25);
...
{
    UserDefault::getInstance()->setIntegerForKey(M_FILE_KEY_HIGHT_SCORE, (int)highScore);
}
...
bool MainLayer::tileMatchesAvailable()
{
    ...
    for (size_t i=0; i!=sizeX; i++) {
        for (size_t j=0; j!=sizeY; j++) {
            TileOfCell *tile=grid.getCellContent((int)i, (int)j);
            if (tile) {
                for (size_t direction=0; direction!=4; direction++) {
                    Cell *cell=map[direction];
                    TileOfCell *other=grid.getCellContent((int)(i+cell->getX()), (int)(j+cell->getY()));
