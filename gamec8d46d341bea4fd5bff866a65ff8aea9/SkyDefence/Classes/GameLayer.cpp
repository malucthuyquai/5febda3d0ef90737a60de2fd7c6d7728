...
bool GameLayer::init()
{
	...
	do 
	{
		...
		//加载background2
		background2=CCSprite::createWithSpriteFrame(CCSpriteFrameCache::getInstance()->getSpriteFrameByName("background.png"));
...
void GameLayer::onTouchMoved(cocos2d::Touch* touch, cocos2d::Event* event)
{
	if (this->planeLayer->isAlive)
	{
		...
		{ 
			...
			Point toPoint=ccpAdd(this->planeLayer->getChildByTag(AIRPLANE)->getPosition(),offSet);
...
void GameLayer::update(float dt)
{
	...
	CCARRAY_FOREACH(this->bulletLayer->m_pAllBullet,bt)
	{
		...
		__Array* enemy1sToDelete=CCArray::create();
	...
	CCARRAY_FOREACH(this->bulletLayer->m_pAllBullet,bt)
	{
		Sprite* bullet=(CCSprite*)bt;

		__Array* enemy2sToDelete=CCArray::create();
	...
	CCARRAY_FOREACH(this->bulletLayer->m_pAllBullet,bt)
	{
		...
		__Array* enemy3sToDelete=CCArray::create();
	...
	Rect airplaneRect=this->planeLayer->getChildByTag(AIRPLANE)->boundingBox();
...
{
	Sprite* normalBomb=Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("bomb.png"));
	Sprite* pressedBomb=Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("bomb.png"));
	if (bigBoomCount<0)
	{
		return;
	}
	...
	else if (bigBoomCount==1)
	{
		...
		if (!this->getChildByTag(TAG_BIGBOOM_MENUITEM))
		{
			MenuItemSprite* pBigBoomItem=MenuItemSprite::create(normalBomb, pressedBomb, this, menu_selector(GameLayer::menuBigBoomCallback));
	...
	else
	{
		if (!this->getChildByTag(TAG_BIGBOOM_MENUITEM))
		{
			MenuItemSprite* pBigBoomItem=MenuItemSprite::create(normalBomb, pressedBomb, CC_CALLBACK_1(GameLayer::menuBigBoomCallback, this));
			pBigBoomItem->setPosition(Point(normalBomb->getContentSize().width/2+10,normalBomb->getContentSize().height/2+10));
			menuBigBoom=CCMenu::create(pBigBoomItem,NULL);
			menuBigBoom->setPosition(Point::ZERO);
			this->addChild(menuBigBoom,0,TAG_BIGBOOM_MENUITEM);
